...
namespace CMS.DAL
{
	...
	public partial class NodeGroup
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "NodeGroup"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from NodeGroup");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into NodeGroup(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update NodeGroup set ");
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from NodeGroup ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		public CMS.Model.NodeGroup GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,name,sort from NodeGroup ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public CMS.Model.NodeGroup DataRowToModel(DataRow row)
		{
			CMS.Model.NodeGroup model=new CMS.Model.NodeGroup();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from NodeGroup T ");
        ...
        public DataSet GetListByPage(string condition, string order, int pageSize, int currentPage, bool isGeneral)
        {
            ...
            string strSql = string.Format(@"SELECT TOP  {0}  *
                            ...
                            WHERE id NOT IN
                                        (
                                        SELECT TOP  {1} id FROM NodeGroup {2} {3}
                                        )
