...
namespace CMS.DAL
{
    ...
    public partial class News
    {
        ...
        public int Add(CMS.Model.News model)
        {
            ...
            strSql.Append("title,source,titleImage,content,keywords,description,addTime,category,recommend,userId,isDel,hit,isSwitch,recommendCount)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@recommendCount", SqlDbType.Int,4)};
            ...
            parameters[13].Value = model.recommendCount;
        ...
        public bool Update(CMS.Model.News model)
        {
            ...
            strSql.Append("recommendCount=@recommendCount");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@title", SqlDbType.VarChar,100),
					new SqlParameter("@source", SqlDbType.VarChar,50),
        ...
        public CMS.Model.News GetModel(int id)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,title,source,titleImage,content,keywords,description,addTime,category,recommend,userId,isDel,hit,isSwitch,recommendCount from News ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
        ...
        public CMS.Model.News DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["recommendCount"] != null && row["recommendCount"].ToString() != "")
                {
                    model.recommendCount = int.Parse(row["recommendCount"].ToString());
                }
            }
            return model;
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,title,source,titleImage,content,keywords,description,addTime,category,recommend,userId,isDel,hit,isSwitch,recommendCount ");
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,title,source,titleImage,content,keywords,description,addTime,category,recommend,userId,isDel,hit,isSwitch,recommendCount ");
            strSql.Append(" FROM News ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        public DataSet GetMyRecommend(int userId)
        {
            ...
            strSql.Append(@"SELECT  * from MyRecommendNews
WHERE   (News.userId = @userId)");
        ...
        public DataSet GetMyRecommendByPage(string condition, string order, int pageSize, int currentPage)
        {
            ...
            string strSql = string.Format(@"SELECT TOP  {0}  *
                            FROM MyRecommendNews
                            WHERE id NOT IN
                                        (
                                        SELECT TOP  {1} id FROM MyRecommendNews {2} {3}
                                        )
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"select count(1) FROM MyRecommendNews
WHERE   userID = "+userId);
        ...
        public List<string> GetAttribute(int id)
        {
            ...
            CMS.DAL.News_Attribute news_attribute_bll = new News_Attribute();
            CMS.DAL.Attribute attributeBll = new Attribute();

            DataSet ds=news_attribute_bll.GetList("newsID=" + id);
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    CMS.Model.Attribute attribute=attributeBll.GetModel(int.Parse(row["attributeID"].ToString()));

                    list.Add(attribute.cssClass);
                }
            }
