...
namespace CMS.DAL
{
	...
	public partial class Node
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "Node"); 
		}
		...
		/// 增加一条数据
		/// </summary>
		public int Add(CMS.Model.Node model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Node(");
			strSql.Append("name,url,sort,groupId,isMenu,menuName)");
			...
			parameters[4].Value = model.isMenu;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(CMS.Model.Node model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public CMS.Model.Node GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,name,url,sort,groupId,isMenu,menuName from Node ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public CMS.Model.Node DataRowToModel(DataRow row)
		{
			CMS.Model.Node model=new CMS.Model.Node();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,name,url,sort,groupId,isMenu,menuName ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" id,name,url,sort,groupId,isMenu,menuName ");
			strSql.Append(" FROM Node ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
        ...
        /// 获得结点所在的组
        /// </summary>
        public DataSet GetNodeGroup(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id, name, sort ");
            strSql.Append(" FROM NodeGroup ");
            if (strWhere.Trim() != "")
            {
                strSql.AppendFormat(" where (id IN(SELECT   groupId FROM      Node WHERE   {0} and isMenu=1 GROUP BY groupId)) " , strWhere);
            }
        ...
        /// 获得带排序的数据列表
        /// </summary>
        public DataSet GetListWithOrder(string strWhere, string order)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,name,url,sort,groupId,isMenu,menuName ");
            strSql.Append(" FROM Node ");
