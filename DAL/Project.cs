...
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2014/2/20 14:52:50   N/A    初版
*
...
namespace CMS.DAL
{
	...
	public partial class Project
	{
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(CMS.Model.Project model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Project(");
            strSql.Append("number,source,title,price,startTime,endTime,titleImage,content,keywords,description,addTime,category,recommend,userId,isDel,hit,isSwitch,isMember)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@isMember", SqlDbType.Bit,1)};
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(CMS.Model.Project model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public CMS.Model.Project GetModel(int id)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,number,source,title,price,startTime,endTime,titleImage,content,keywords,description,addTime,category,recommend,userId,isDel,hit,isSwitch,isMember from Project ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
        ...
        /// 得到一个对象实体
        /// </summary>
        public CMS.Model.Project DataRowToModel(DataRow row)
        {
            CMS.Model.Project model = new CMS.Model.Project();
            if (row != null)
            {
                ...
                {
                    if ((row["isMember"].ToString() == "1") || (row["isMember"].ToString().ToLower() == "true"))
                    {
                        model.isMember = true;
                    }
                    else
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,number,source,title,price,startTime,endTime,titleImage,content,keywords,description,addTime,category,recommend,userId,isDel,hit,isSwitch,isMember ");
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,number,source,title,price,startTime,endTime,titleImage,content,keywords,description,addTime,category,recommend,userId,isDel,hit,isSwitch,isMember ");
            strSql.Append(" FROM Project ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        public DataSet GetListByPage(string condition, string order, int pageSize, int currentPage, bool isGeneral)
        {
            ...
            string strSql = string.Format(@"SELECT TOP  {0}  *
                            ...
                            WHERE id NOT IN
                                        (
                                        SELECT TOP  {1} id FROM Project {2} {3}
                                        )
