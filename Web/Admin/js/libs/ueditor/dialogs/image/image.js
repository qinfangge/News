    ...
    function insertSearch(id, catchRemote) {
        ...
        for (var i = 0, ci; ci = imgs[i++];) {
            if (ci.getAttribute("selected")) {
                ...
                img.src = url;
                img._src = url;
                var bigUrl=ci.getAttribute("_src", 2)
                if (bigUrl!=undefined) {
                    img.src = bigUrl;
                    img._src = bigUrl;
                }
                        ...
                        ajax.request(editor.options.imageManagerUrl, {
                            ...
                            onsuccess:function (xhr) {
                                ...
                                for (var k = 0, ci; ci = imageUrls[k++];) {
                                    ...
                                    var images = ci.split("big_small_big");//大图与小图的区分;
                                    var smallUrl = images[0];
                                    var bigUrl = images[1];
                                   ...
                                   // img.setAttribute("_src", editor.options.imageManagerPath + ci.replace(/\s+|\s+/ig, ""));
                                    ...
                                    img.setAttribute(k < 35 ? "src" : "lazy_src", editor.options.imageManagerPath + smallUrl.replace(/\s+|\s+/ig, ""));

                                    img.setAttribute("_src", editor.options.imageManagerPath + bigUrl.replace(/\s+|\s+/ig, ""));
